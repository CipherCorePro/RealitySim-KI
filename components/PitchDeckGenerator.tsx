...
import { FileContents, PitchDeck, PitchDeckJob } from '../types';
import Icon, { IconProps } from './Icon';
import { runPitchDeckGeneration } from '../services/geminiService';
...
const PitchDeckGenerator: React.FC<PitchDeckGeneratorProps> = ({ downloadFile }) => {
    const { t, language } = useTranslation();
    const [job, setJob] = useState<PitchDeckJob>({ status: 'idle', progress: 0, logs: [], error: null, result: null });
    ...
    const handleGenerate = async () => {
        ...
        try {
            for await (const update of runPitchDeckGeneration(parsedFileContents, language)) {
                setJob(prev => ({
                    ...prev,
    ...
    return (
        <div className="h-full flex flex-col items-center justify-center p-4">
            <div className="w-full max-w-5xl bg-slate-800/50 rounded-xl shadow-2xl p-8 flex flex-col gap-8">
                <div>
                    <h1 className="text-4xl font-extrabold text-white text-center mb-2">{t('pitchDeckGenerator')}</h1>
                    <p className="text-center text-slate-400 mb-6 max-w-2xl mx-auto">{t('uploadPitchDeckDocsDescription')}</p>
                ...
                {job.status === 'completed' && job.result && <PitchDeckPreview deck={job.result} downloadFile={downloadFile} onReset={handleReset} />}
...
const FileUploader: React.FC<{onFileParse: (files: FileList) => void; isProcessing: boolean; selectedFileNames: string[]}> = ({ onFileParse, isProcessing, selectedFileNames }) => {
    ...
    return (
        <div>
            ...
            >
                <input type="file" multiple ref={fileInputRef} onChange={e => handleFiles(e.target.files)} className="hidden" accept=".pdf,.doc,.docx,.xls,.xlsx,.md,.html,.txt,.jpg,.jpeg,.png" disabled={isProcessing} />
                <Icon type="upload" className="w-10 h-10 mx-auto text-slate-500 mb-2" />
                <p className="font-semibold text-slate-300">{t('uploadPitchDeckDocs')}</p>
            </div>
...
const JobProgressView: React.FC<{job: PitchDeckJob}> = ({ job }) => {
    const { t } = useTranslation();
    ...
    const statusMessage = lastLog ? `${lastLog.agent}: ${t(lastLog.message as string)}` : t(job.status);
    ...
    return (
        <div className="bg-slate-900/50 p-4 rounded-lg">
            ...
            <p className="text-slate-400 text-sm mb-4">{t(job.status === 'generating' ? 'generatingPitchDeck' : statusMessage)}</p>
...
const PitchDeckPreview: React.FC<{ deck: PitchDeck; downloadFile: (content: string, fileName: string, mimeType: string) => void; onReset: () => void; }> = ({ deck, downloadFile, onReset }) => {
    const { t } = useTranslation();

    return (
        <div className="bg-slate-900/50 p-4 rounded-lg">
            <div className="flex justify-between items-center mb-4">
                <h3 className="text-lg font-semibold text-slate-200">{t('pitchDeckPreview')}</h3>
                <div className="flex items-center gap-2">
                    <button onClick={() => downloadFile(deck.html, 'pitch-deck.html', 'text/html')} className="px-3 py-1.5 text-sm bg-indigo-600 hover:bg-indigo-500 rounded-md flex items-center gap-2"><Icon type="html" className="w-4 h-4"/> {t('downloadHTML')}</button>
                    <button onClick={() => downloadFile(deck.markdown, 'pitch-deck.md', 'text/markdown')} className="px-3 py-1.5 text-sm bg-slate-600 hover:bg-slate-500 rounded-md flex items-center gap-2"><Icon type="md" className="w-4 h-4"/> {t('downloadMD')}</button>
             ...
             <div className="bg-white p-1 rounded-md max-h-[60vh] overflow-y-auto">
                <iframe
                    srcDoc={deck.html}
                    className="w-full h-[58vh] border-0"
                    sandbox="allow-scripts allow-same-origin"
                    title={t('pitchDeckPreview')}
                />
            </div>
        </div>
    );
