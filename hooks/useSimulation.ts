...
const sanitizeAndCreateAgents = (generatedAgents: any[], worldState: WorldState): Agent[] => {
    const existingNames = new Set(worldState.agents.map(a => a.name));

    return generatedAgents.map((geminiAgent: any, index: number) => {
        let name = String(geminiAgent.name || '').trim();

        if (!name || name.split(' ').length < 2 || existingNames.has(name)) {
            name = generateUniqueFullName(existingNames);
        }
...
export const useSimulation = () => {
    ...
    const addLog = useCallback((logEntry: LogEntry) => {
        ...
        setLogs(prevLogs => [timedLog, ...prevLogs]);
    }, []);
    ...
    const handleRunSteps = useCallback(async (steps: number) => {
      ...
      try {
          for (let i = 0; i < steps; i++) {
            ...
            // Reversing is important because addLog prepends. This ensures logs from a single step appear in chronological order.
            newLogs.reverse().forEach(log => addLog(log));
            
            // Short delay to allow the UI to re-render and give the user a sense of progression.
