import type { WorldState, Agent, Entity, Action, EnvironmentState, Beliefs, Resonance, LogEntry, Relationship, Culture, ActionExecutionResult, Religion, Personality, Skills, Trauma, Goal, Law, TradeOffer, ItemType, SocialMemoryEntry, PsychoReport } from '../types';
import { 
    ...
    MAX_SOCIAL_MEMORIES, SKILL_TYPES, ADULTHOOD_MAX_AGE
} from '../constants';
...
export class RealityEngine {
    ...
    private chooseAction(agent: Agent): Action | null {
        ...
        const eatAction = this.actions.get("Eat Food");
        if (agent.hunger > 60 && eatAction) actionScores.set(eatAction, (actionScores.get(eatAction) || 0) + agent.hunger);
        const drinkAction = this.actions.get("Drink Water");
        if (agent.thirst > 60 && drinkAction) actionScores.set(drinkAction, (actionScores.get(drinkAction) || 0) + agent.thirst);
        ...
        const workAction = this.actions.get("Work for money");
        if (agent.currency < 20 && workAction) actionScores.set(workAction, (actionScores.get(workAction) || 0) + (80 - agent.currency));

        // Apply unconscious modifiers
        if (agent.unconsciousState) {
            if ((agent.unconsciousState['suppressed_melancholy'] || 0) > 0.5) {
                const talkAction = this.actions.get("Talk");
                if (talkAction) actionScores.set(talkAction, (actionScores.get(talkAction) || 0) - 20); // Less likely to talk
                if (restAction) actionScores.set(restAction, (actionScores.get(restAction) || 0) + 30); // More likely to rest
            }
            if ((agent.unconsciousState['fear_of_being_a_burden'] || 0) > 0.5) {
                if ((agent.inventory['food'] || 0) > 10) {
                     const gatherFoodAction = this.actions.get("Gather Food");
                     if (gatherFoodAction) actionScores.set(gatherFoodAction, (actionScores.get(gatherFoodAction) || 0) - 25);
                }
            }
            if ((agent.unconsciousState['hidden_aggression'] || 0) > 0.6) {
                 const fightAction = this.actions.get("Fight");
                 if (fightAction) actionScores.set(fightAction, (actionScores.get(fightAction) || 0) + 40);
            }
        }
        ...
        agent.goals.forEach(goal => {
            if (goal.type === 'avengeRival' && goal.targetId) {
                ...
                const fightAction = this.actions.get("Fight");
                if (rival && fightAction && Math.sqrt(Math.pow(agent.x - rival.x, 2) + Math.pow(agent.y - rival.y, 2)) < PROXIMITY_DISTANCE_THRESHOLD) {
                    actionScores.set(fightAction, (actionScores.get(fightAction) || 0) + 120);
                }
            }
            if (goal.type === 'becomeLeader') {
                const runAction = this.actions.get("Run for Election");
                if (runAction) actionScores.set(runAction, (actionScores.get(runAction) || 0) + 90);
            }
            if (goal.type === 'mentorYoungAgent') {
                const mentorAction = this.actions.get("Mentor young agent");
                if (mentorAction) actionScores.set(mentorAction, (actionScores.get(mentorAction) || 0) + 110);
            }
        });
        
        if (agent.stress > 80 && agent.personality.neuroticism > 0.6) {
            const seekHelpAction = this.actions.get("Seek Counseling");
            if(seekHelpAction) actionScores.set(seekHelpAction, (actionScores.get(seekHelpAction) || 0) + agent.stress);
        }

        if (agent.role === 'Scientist') {
            const researchAction = this.actions.get("Research");
            if (researchAction) actionScores.set(researchAction, (actionScores.get(researchAction) || 0) + 70);
        }
        if (agent.role === 'Guard') {
            const patrolAction = this.actions.get("Patrol");
            if (patrolAction) actionScores.set(patrolAction, (actionScores.get(patrolAction) || 0) + 80);
        }
        if (agent.role === 'Crafter') {
            ...
            if(craftable) {
                const craftAction = this.actions.get(craftable.name);
                if (craftAction) actionScores.set(craftAction, (actionScores.get(craftAction) || 0) + 60);
            }
        }
        if (agent.role === 'Counselor') {
            const provideHelpAction = this.actions.get("Provide Counseling");
            if(provideHelpAction) actionScores.set(provideHelpAction, (actionScores.get(provideHelpAction) || 0) + 85);
        }
