...
import type { WorldState, Agent, Entity, EnvironmentState, LogEntry, Culture, Law, Personality, PsychoReport, Election } from './types';
...
import { generateActionFromPrompt, generateWorld, generateAgents, generateEntities, LmStudioError, generatePsychoanalysis } from './services/geminiService';
import { BrainCircuit, Cpu, Zap, Microscope, Boxes, Trash2, Settings, X, Globe, Users, PlusSquare, Apple, Droplet, Log, Hammer, Home, Vote } from './components/IconComponents';
...
export default function App() {
  ...
  // New state for Psychoanalysis feature
  ...
  const handleGeneratePsychoanalysis = useCallback(async (agent: Agent) => {
      ...
      try {
          ...
          if (report) {
              ...
              const { logs: applyLogs } = engine.applyPsychoanalysis(agent.id, report);
              setWorldState({ ...engine.getState() });
              applyLogs.forEach(log => addLog(log));
          } else {
              addRawLog("Failed to generate psychological profile.");
              setIsPsychoanalysisModalOpen(false);
          }
      } catch (error) {
          if (error instanceof LmStudioError && error.translationKey) {
              addRawLog(t(error.translationKey as TranslationKey));
              ...
              const errorMessage = error instanceof Error ? error.message : String(error);
              addRawLog(t('log_aiError', { error: `Psychoanalysis failed: ${errorMessage}` }));
          }
          setIsPsychoanalysisModalOpen(false);
      } finally {
          setIsGeneratingAnalysis(false);
      }
  }, [addRawLog, t, isAiConfigured, language, worldState, engine, addLog]);
    ...
    <div className="min-h-screen bg-slate-900 text-slate-300 font-sans">
      ...
      <main className="grid grid-cols-12 gap-4 p-4">
        ...
        <div className="col-span-12 lg:col-span-6 space-y-4">
          {selectedAgent ? (
             <AgentCard agent={selectedAgent} allAgents={worldState.agents} cultures={worldState.cultures || []} religions={worldState.religions || []} onPrompt={handlePrompt} onGeneratePsychoanalysis={handleGeneratePsychoanalysis} />
          ) : (
                    ...
                    <div className="bg-slate-850 p-4 rounded-lg border border-slate-700">
                        ...
                        <ul className="space-y-2 text-sm">
                            {Object.entries(worldState.environment).map(([key, value]) => {
                                if (key === 'election') {
                                    const election = value as Election | null;
